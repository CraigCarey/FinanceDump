...
def get_trust_basics(soup, idx):
    trust_basics = soup.find_all(
        class_='factsheet-table table-no-border spacer-bottom')[0]
...
def get_symbol_data(soup):
    ...
    annual_mgmt_charge = get_trust_basics(
        soup, annual_mgmt_charge_idx).replace(' ', '')
    performance_fee = get_trust_basics(
        soup, performance_fee_idx).replace(' ', '')
    ongoing_charge = get_trust_basics(
        soup, ongoing_charge_idx).replace(' ', '')
    ...
    trust_data = TrustData(nav, nav_premium, volume, div_yld, annual_mgmt_charge, performance_fee,
                           ongoing_charge, total_assets, gross_gearing, market_cap, structure, domicile, isin, objective)
...
def main():
    ...
    df_columns = ['symbol', 'name', 'nav', 'nav_premium', 'volume', 'div_yld', 'annual_mgmt_charge', 'performance_fee',
