...
batch_size = 2
max_threads = (
    7  # Somewhere between 10 and 15 threads with batch_size of 10 seems to be allowed
)
...
def get_roc(ticker):
    return get_ebit(ticker) / (
        get_net_working_capital(ticker) + get_fixed_assets(ticker)
    )
...
def get_net_working_capital(ticker):
    return max(
        0,
        get_totalCurrentAssets(ticker)
        - get_excess_cash(ticker)
        - get_accountsPayable(ticker),
    )
...
def get_fixed_assets(ticker):
    ...
    return (
        get_totalAssets(ticker)
        - get_totalCurrentAssets(ticker)
        - get_intangibles(ticker)
        - get_goodwill(ticker)
    )


def get_excess_cash(ticker):
    return get_cash(ticker) - max(
        0,
        get_totalCurrentLiabilities(ticker)
        - get_totalCurrentAssets(ticker)
        + get_cash(ticker),
    )
...
def get_financials_date(ticker):
    ...
    balance_date = date(
        int(balance_date[0]), int(balance_date[1]), int(balance_date[2])
    )
...
def rank_stocks(db):
    ...
    pd.DataFrame.from_records(data=query.fetchall(), columns=cols).to_csv(
        "stock_info.csv"
    )
...
def validate_tickers(tickers, cap_dict, batch_sz=batch_size, newonly=False):
    ...
    for i in range(batches + 1):
        ticker_sublist = ticker_keys[
            i * batch_sz : min((i + 1) * batch_sz, len(ticker_keys))
        ]
        if (
            len(ticker_sublist) == 0
        ):  # This is for when the batch_size evenly divides into the ticker_dict size
            break

        thread = threading.Thread(
            target=validate_tickers_thread,
            args=(ticker_sublist, tickers, cap_dict, i, newonly),
        )
...
def validate_tickers_thread(ticker_keys, tickers, cap_dict, batch_no, newonly):
    ...
    if newonly:
        ticker_keys = [
            ticker for ticker in ticker_keys if tickers[ticker] == TICKER_NOT_VALIDATED
        ]
    ...
    print(
        f"Batch {batch_no + 1}: Retrieving volume and price information from Yahoo Finance..."
    )
    ...
    for ticker, value in avg_ten_day_volume.items():
        if (
            ticker is None
            or value is None
            or ticker not in current_price
            or current_price[ticker] is None
        ):
    ...
    for ticker in ticker_keys:
        if ticker in missing_data:
            ...
            avg_ten_day_dollar_volume[ticker] > min_dollar_volume
            and avg_three_month_dollar_volume[ticker] > min_dollar_volume
        ):
    ...
    if len(new_ticker_keys) == 0:
        ...
        end_loop = time.time()
        print(
            f"Time elapsed for ticker validation, batch {batch_no + 1}: {end_loop - start_loop}"
        )
        print()
        return
    ...
    print(
        f"Batch {batch_no + 1}: Retrieving market cap information from Yahoo Finance..."
    )
...
def retrieve_data(batch_sz, ticker_keys, metric, file_name, data_dict):
    ...
    for i in range(batches + 1):
        ...
        thread = threading.Thread(
            target=create_retrieve_thread,
            args=(ticker_sublist, metric, file_name, data_dict, i),
        )
        thread_jobs.append(thread)
...
def clean_tickers():
    ...
    for ticker, value in ticker_dict.items():
        ...
        if value == TICKER_REMOVE or (
            value == TICKER_VALID
            and (
                ticker not in balance_sheet
                or ticker not in income_statement
                or ticker not in market_cap
                or balance_sheet[ticker] is None
                or income_statement[ticker] is None
                or market_cap[ticker] is None
                or balance_sheet[ticker] == []
                or income_statement[ticker] == []
...
def create_process(batch_sz, p_tickers, p_id):
    ...
    market_cap = {}
    retrieve_data(
        batch_sz, p_tickers[0], "balance", f"{fn_balance}_{p_id}", balance_sheet
    )
    retrieve_data(
        batch_sz, p_tickers[1], "income", f"{fn_income}_{p_id}", income_statement
    )
...
def scrape_sector(ticker):
    try:
        ...
        print(
            f'Ticker: {ticker}, Error in finding profile: {e}. Setting sector, industry, and country as "TBD"'
        )
    ...
    try:
        ...
        print(
            f'Ticker: {ticker}, Error in finding country: {e}. Setting country as "TBD"'
        )
        country = "TBD"
    ...
    return {
        ticker.upper(): {"sector": sector, "industry": industry, "country": country}
    }
...
def scrape_sector_all(tickers, batch_sz=0):
    if batch_sz == 0:
        ...
        print("Creating Threads...")
        for i in range(batches + 1):
            ticker_sublist = tickers[
                i * batch_sz : min((i + 1) * batch_sz, len(tickers))
            ]
            if (
                len(ticker_sublist) == 0
            ):  # This is for when the batch_size evenly divides into the ticker_dict size
...
if __name__ == "__main__":
    ...
    if args.refresh_tickers:
        ...
        fhr = open("nasdaqlisted.txt")
        lines = fhr.readlines()
        for line in lines:
            fields = line.split("|")
            ticker_dict[fields[0]] = TICKER_NOT_VALIDATED
        fhr.close()
        fhr = open("otherlisted.txt")
        lines = fhr.readlines()
        for line in lines:
            fields = line.split("|")
            ticker_dict[fields[0]] = TICKER_NOT_VALIDATED
    ...
    if debug:
        ticker_dict = {
            key: ticker_dict[key] for key in list(ticker_dict.keys())[0:20]
        }  # For debugging purposes, when we want a smaller ticker_dict to work with
    ...
    if args.continue_refresh:
        ...
        for i in range(args.n_processes):
            ...
            p_balance_list.append(
                balance_sublist[
                    i * p_size : min((i + 1) * p_size, len(balance_sublist))
                ]
            )
        ...
        for i in range(args.n_processes):
            p_income_list.append(
                income_sublist[i * p_size : min((i + 1) * p_size, len(income_sublist))]
            )
        ...
        for i in range(args.n_processes):
            p_cap_list.append(
                cap_sublist[i * p_size : min((i + 1) * p_size, len(cap_sublist))]
            )
        ...
        for i in range(args.n_processes):
            process = Process(
                target=create_process, args=(batch_size, p_ticker_list[i], i)
            )
            jobs.append(process)
    ...
    if args.retrieve_sector != -1:
        print(
            f"Retrieving sector, industry, and country info.... Batch Size: {args.retrieve_sector}"
        )
        sector_dict = {}
    ...
    for matched_ticker in get_valid_ticker_list():
        if (
            matched_ticker in balance_sheet
            and matched_ticker in income_statement
            and matched_ticker in market_cap
        ):
